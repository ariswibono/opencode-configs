...
- `humanizer`: remove signs of AI-generated writing from text — 24 pattern categories plus voice injection.
- `git-commit`: automated conventional commits with diff analysis, intelligent staging, and message generation.
- `release`: automate screenpipe monorepo releases — bump versions, trigger GitHub Actions for app/CLI/MCP/JS packages.
- `capability-evolver`: self-evolution engine for AI agents — analyze runtime logs, detect failures, generate improvements via GEP protocol.
- `skill-creator`: create, modify, improve, and benchmark skills — full workflow from intent capture to eval iteration to description optimization.
